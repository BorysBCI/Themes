...
(function (BasisBuilderApp, $, _, basisHTMLBuilderData) {
	'use strict';
	...
	var BasisAdmin = function () {
		...
		function hideHeaderOption() {
			var compiled = _.template('<p><strong>Minimal Mode</strong></p><label id="basis-hide-header-option" for="basis-hide-header" style="display:none;"><input type="checkbox" id="basis-hide-header" name="basis-hide-header" value="1" <%= hideHeaderChecked %> /> <%- hideHeaderLabel %></label>'),
				$loadAfter = $('#menu_order', cache.$pageParentDiv).parent(); // Grabs the "Order" input bounding "p" element

			$loadAfter.after( compiled(basisHTMLBuilderData) );
...
})(BasisBuilderApp, jQuery, _, basisHTMLBuilderData);