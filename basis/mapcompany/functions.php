...
function add_user_custom_meta($user_id) {
	...
	//$admin_email = "info@tandemnsi.com";
	$admin_email = array('info@tandemnsi.com', 'borysmar@gmail.com', $user_info->user_email);
...
/* function elh_insert_into_db() {
    ...
    /* if ( isset( $_POST["submit_form"] ) && $_POST["nameCompany"] != "" && $_POST["address"] != "" && $_POST["website"] != "" && $_POST["Category"] != "" && $_POST["Category2"] != "" && $_POST["liner1"] != ""&& $_POST["address_1"] != ""&& $_POST["address_2"] != "" && $_POST["city"] != "" && $_POST["state"] != "" && $_POST["zip_code"] != "" && $_POST["lat"] != "" && $_POST["lng"] != "" )  {
        $table = $wpdb->DB_Company;
        $nameCompany = strip_tags($_POST["nameCompany"], "");
        $address = strip_tags($_POST["address"], "");
        $website = strip_tags($_POST["website"], "");
        $Category = strip_tags($_POST["Category"], "");
        $Category2 = strip_tags($_POST["Category2"], "");
        $liner1 = strip_tags($_POST["liner1"], "");
        ...
        $html = "<p>Your name <strong>$nameCompany</strong> was successfully recorded. Thanks!!</p>";
    }
...
function theme_settings_page() {
 ...
 <tr>
			...
			$result=mysql_query("SELECT * FROM DB_Company ORDER BY nameCompany ");
			...
			while($test = mysql_fetch_array($result))
			{
				...
				echo"<td> <a href ='http://www.tandemnsi.com/wp-content/themes/basis/delete.php?Id=$id'><center>Delete</center></a>";
...
add_menu_page( __( 'User' .'Manager' ), __( 'User' .'Manager' ), 'manage_options', 'user_settings', 'theme_user_settings_page');
}
...
<table border="1">
			...
			//$result=mysql_query("SELECT ID, user_login, user_pass, user_email, m1.meta_value AS display_name, m2.meta_value AS user_nicename, m3.meta_value AS user_organization FROM wp_users u1 JOIN wp_usermeta m1 ON (m1.user_id = u1.ID AND m1.meta_key =  'display_name') JOIN wp_usermeta m2 ON (m2.user_id = u1.ID AND m2.meta_key = 'user_nicename') JOIN wp_usermeta m3 ON (m3.user_id = u1.ID AND m3.meta_key = 'user_organization') where m1.meta_key = 'first_name'");
			//$result=mysql_query("Select * from wp_users, wp_usermeta where ID = ID");
			$result=mysql_query("SELECT a.id, b1.meta_value AS display_name, b2.meta_value as user_nicename, b3.meta_value as user_organization, b4.meta_value as user_tilte, b5.meta_value as user_street_address, b6.meta_value as user_city,  b7.meta_value as user_state, b8.meta_value as user_phone, b9.meta_value as page_key, b10.meta_value as page_id 
FROM wp_users a
...
WHERE (b1.meta_value LIKE '%st%' OR b2.meta_value LIKE '%st%' OR b3.meta_value LIKE '%st%' OR b4.meta_value LIKE '%st%' OR b5.meta_value LIKE '%st%' OR b6.meta_value LIKE '%st%' OR b7.meta_value LIKE '%st%' OR b8.meta_value LIKE '%st%' OR b9.meta_value LIKE '%st%' OR b10.meta_value LIKE '%st%')");
			...
			while($test = mysql_fetch_array($result))
			{
				...
				echo "<tr align='center'>";	
				echo"<td><font color='black'>" .$test['id']."</font></td>";
				echo"<td><font color='black'>" .$test['display_name']."</font></td>";
				echo"<td><font color='black'>". $test['user_nicename']. "</font></td>";
				echo"<td><font color='black'>". $test['user_organization']. "</font></td>";
				echo"<td><font color='black'>". $test['user_tilte']. "</font></td>";	
				echo"<td><font color='black'>". $test['user_street_address']. "</font></td>";	

				echo"<td><font color='black'>". $test['user_city']. "</font></td>";	
				echo"<td><font color='black'>". $test['user_state']. "</font></td>";	
				echo"<td><font color='black'>". $test['user_phone']. "</font></td>";	
				echo"<td><font color='black'>". $test['page_key']. "</font></td>";	
				echo"<td><font color='black'>". $test['page_id']. "</font></td>";	

				//echo"<td> <a href ='http://www.tandemnsi.com/wp-content/themes/basis/uview.php?ID=$id'>Edit</a>";
				echo"<td> <a href ='http://www.tandemnsi.com/wp-content/themes/basis/udelete.php?id=$id'><center>Delete</center></a>";
